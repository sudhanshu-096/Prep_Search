...
const TandCComponent = ({terms, setTerms}) => {
  return (
    <View style={{flex:1, backgroundColor:"#032D62"}}>
    <ScrollView contentContainerStyle={{paddingBottom:50, height:height}}>
    <View style={{flexDirection:"row",justifyContent:"space-between", alignItems:"center"}}>
      <BackButton bgColor="#FDBA31"color="#000" onPress={()=>{setTerms(false)}}/>
         <Text style={{color:"#fff",fontSize:isSmallScreen?22:24, fontWeight:"600", textAlign:"center"}}>Terms and Conditions</Text>
         <Text></Text>
         <Text></Text>
      </View>
      ...
      <View style={{width:width*0.9,alignSelf:"center" }}>
          <Text style={{fontWeight:"400",color:"#fff", lineHeight:22,fontSize:isSmallScreen?14:16, textAlign:"justify"}}>This account is for verified college coaches and their direct support staff only.
...
PrepSearch database and app services are available to verified employees of Colleges and Universities. We do not sell to individuals. All purchases, rosters, evaluations, and work remain within the school account. Once you leave a program, you will no longer have access to that school’s PrepSearch account and will need to complete a new registration.</Text>
      ...
      <View style={{width:width*0.95, alignItems:"left" , marginLeft:10,}}>
        <Text style={{textAlign:"justify",lineHeight:20,color:"#fff"}}>{' \u25CF You are a registered staff member for your respective program '}</Text>
        <Text style={{textAlign:"justify",lineHeight:20,color:"#fff"}}>{' \u25CF You will not share your login with anyone else, including other staff members '}</Text>
        <Text style={{textAlign:"justify",lineHeight:20,color:"#fff"}}>{' \u25CF You will immediately remove accounts of former staff members who stop working at you school or ask PrepSearch support to do it.'}</Text>
      </View>
    </ScrollView>
    </View>
  )
